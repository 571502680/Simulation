...
#-*- coding: utf-8 -*-
"""
这里面尝试直接使用Realsense进行DenseFusion的识别,直接图片resize,其他的都不管
"""
...
class DenseFusion_Detector:
    def __init__(self,model_path=None,refine_model_path=None,segnet_path=None,init_node=False):
        ...
        #6:信息发布
        self.poses_pub=rospy.Publisher("/DenseFusionInfer/PoseinWorld",ModelStates)
    ...
    def pub_pose_array(self):
        ...
        """
        read_Data=Read_Data.Read_Data()
        read_Data.begin_get_images("kinect")
        rate=rospy.Rate(10)
        with torch.no_grad():
            while not rospy.is_shutdown():
                if read_Data.bgr_image_k is not None and read_Data.depth_image_k is not None:
                    pose_results=self.get_pose(read_Data.bgr_image_k,read_Data.depth_image_k)
