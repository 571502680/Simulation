...
class Read_Data:
    def __init__(self,init_node=False,simulator="sapien"):
        if init_node:
            rospy.init_node("Read_Data")

        #读取图片信息
        self.bgr_image_k=None
        self.depth_image_k=None
        self.camera_info_k=None
        self.bgr_image_r=None
        self.depth_image_r=None
        self.camera_info_r=None
        
        #默认好的摄像头信息
        self.camera_info_k=np.array([[1120.1199067175087, 0.0, 640.5], [0.0, 1120.1199067175087], [360.5, 0.0, 0.0, 1.0]])
    ...
    def depth_process_callback_k(self,data,see_image=False):
        ...
        try:
            ...
            self.depth_image_k=cv_image*10000
            self.depth_image_k[np.isnan(self.depth_image_k)]=0
        except CvBridgeError as e:
            print("[Error] depth_process_callback occur error {}".format(e))
            return

        if see_image:
            if self.depth_image_k is  None:
                print("[Warning] Can not get the depth_image")
                return
            cv_image=self.depth_image_k.copy()
    ...
    def image_process_callback_k(self,data,see_image=False,read_HSV=False):
        ...
        """
        try:
            self.bgr_image_k=self.bridge.imgmsg_to_cv2(data,"bgr8")
        except CvBridgeError as e:
        ...
        if see_image:
            if self.bgr_image_k is  None:
                print("[Warning] Can not get the bgr_image")
                return
            cv_image=self.bgr_image_k.copy()
    ...
    def depth_process_callback_r(self,data,see_image=False):
        ...
        try:
            cv_image=self.bridge.imgmsg_to_cv2(data,'32FC1')
            self.depth_image_r=cv_image*10000
            self.depth_image_r[np.isnan(self.depth_image_r)]=0
        except CvBridgeError as e:
            print("[Error] depth_process_callback occur error {}".format(e))
            return

        if see_image:
            if self.depth_image_r is  None:
                print("[Warning] Can not get the depth_image")
                return
            cv_image=self.depth_image_r.copy()
            cv_image=cv_image*1000
            ROI=cv.inRange(cv_image,0,1500)#对于太远的去掉,省得看的并不明显
            cv_image=cv_image*ROI/255
            cv.normalize(cv_image,cv_image,255,0,cv.NORM_MINMAX)
    ...
    def image_process_callback_r(self,data,see_image=False,read_HSV=False):
        ...
        """
        try:
            self.bgr_image_r=self.bridge.imgmsg_to_cv2(data,"bgr8")
        except CvBridgeError as e:
            print("[Error] image_process_callback occur error {}".format(e))
            return
        ...
        if see_image:
            if self.bgr_image_r is  None:
                print("[Warning] Can not get the bgr_image")
                return
            cv_image=self.bgr_image_r.copy()
            self.read_image_bgr=cv_image
            cv.namedWindow("image_get",cv.WINDOW_NORMAL)
            cv.imshow("image_get",cv_image)
            if read_HSV:
    ...
    def begin_get_images(self,camera="kinect"):
        """
        开启相机读取操作
        :param camera:指定相机名称,可以为kinect,realsense,both
        :return:
        """
        if camera=="kinect":
    ...
    def get_images(self,camera="kinect"):
        """
        指定相机,进行图片读取
        :param camera: 相机名称,为realsense或者kinect,如果both就是都获取
        :return:
        """
        if camera=="kinect":
            if self.bgr_image_k is not None and self.depth_image_k is not None:
                return self.bgr_image_k,self.depth_image_k
            else:
                if self.bgr_image_k is None:
                    print("[Warning] bgr_image_k is None")
                if self.depth_image_k is None:
                    print("[Warning] depth_image_k is None")
                time.sleep(0.5)#避免输出太快
                return None,None
        elif camera=="realsense":
            if self.bgr_image_r is not None and self.depth_image_r is not None:
                return self.bgr_image_r,self.depth_image_r
            else:
                if self.bgr_image_r is None:
                    print("[Warning] bgr_image_r is None")
                if self.depth_image_r is None:
                    print("[Warning] depth_image_r is None")
                time.sleep(0.5)#避免输出太快
                return None,None
        elif camera=="both":
            if self.bgr_image_r is not None and self.depth_image_r is not None and self.bgr_image_k is not None and self.depth_image_k is not None:
                return self.bgr_image_r,self.depth_image_r,self.bgr_image_k,self.depth_image_k
            else:
                print("[Waring] both images is not get")
                time.sleep(0.5)#避免输出太快
...
def test_read_image_two_camera():
    """
    用于测试读取图片函数
    @return:
    """
    read_Data=Read_Data(init_node=True)
    read_Data.begin_get_images_k()
    read_Data.begin_get_images_r()
    while not rospy.is_shutdown():
        bgr_image_k,depth_image_k,bgr_image_r,depth_image_r=read_Data.get_images("both")
        if bgr_image_k is not None and bgr_image_k is not None:
            cv.imshow("bgr_iamge_r",bgr_image_r)
            cv.imshow("bgr_iamge_k",bgr_image_k)
            cv.waitKey(3)

def test_get_camera_info():
    read_Data=Read_Data(init_node=True)
    read_Data.begin_get_camera_info_k()
    while not rospy.is_shutdown():
        if read_Data.camera_info_k is not None:
            break
        else:
