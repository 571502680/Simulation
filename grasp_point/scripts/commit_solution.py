...
class CommitSolution(object):
    def __init__(self, name):
        ...
        #grasp point
	self.Need_to_Adjust = {'gelatin_box':[0,0,0,0,math.pi,math.pi/2],'potted_meat_can':[0,0,0,0,math.pi,math.pi/2],'mustard_bottle':[0,0,0,0,3*math.pi/4,math.pi/2],
    ...
    def execute_callback(self, goal):
		...
		if name in self.Need_to_Adjust:
        	    ...
        	    grasp_pose = robot.get_pickpose_from_pose(origin_pose,x=param[0],y=param[1],z=param[2],degreeR=param[3],degreeP=param[4],degreeY=param[5])
		else:
		...
		if name in self.Need_to_Adjust:
		    try:
		    	param = self.Need_to_Adjust.get(name)
                    	place_pose=robot.get_pickpose_from_pose(goal_pose,x=param[0],y=param[1],z=param[2],degreeR=param[3],degreeP=param[4],degreeY=param[5])
		    except:
