...
CENTERNET_PATH =os.path.dirname(__file__)+"/lib_tenclass"#导入对应的lib文件
...
class CenterDetect():
            ...
            for category in self.detections:#用于获取detections中的id号
                for result in self.detections[category]:#用于获取detections中的每个类
                    if result[4]>confidence:
                        ...
                        if show_confidence:
                            show_text_info="{} {:.3f}".format(category,score)
    ...
    def fileter_detections_allcategory(self,confidence=0.5,detections=None):
        """
        用于过滤识别结果
        :param confidence: 置信度
        :param category: 识别种类
